...
def main():
    ...
    # 必要な列の存在確認
    required_columns = ['torque_x', 'distance', 'tilt_angle', 'wall_spacing', 'variance_torque_x', 'sample_count']
    ...
    # NaNを含む行を削除
    df_clean = df.dropna(subset=['torque_x', 'distance', 'tilt_angle', 'wall_spacing', 'variance_torque_x', 'sample_count'])
    ...
    for tilt_angle in unique_tilt_angles:
        for wall_spacing in unique_wall_spacings:
            ...
            if len(pair_data) > 0:
                # 線とマーカーでプロット
                plt.plot(pair_data['distance'], -pair_data['torque_x_mean'], 
    ...
    plt.ylabel('壁効果モーメント [Nm]', fontsize=18)
